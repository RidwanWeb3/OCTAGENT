  ...
  return (
    <div className="relative border-y border-border/40 bg-background/60 backdrop-blur-md overflow-hidden mask-fade-x">
      <div className="flex whitespace-nowrap py-3 animate-ticker" style={{ width: "max-content" }}>
        {loop.length === 0 && (
          <span className="px-6 text-xs font-mono text-muted-foreground tracking-widest">STREAMING LIVE MARKET DATA…</span>
        )}
        {loop.map((c, i) => {
          const up = (c.price_change_percentage_24h ?? 0) >= 0;
          return (
            <span key={`${c.id}-${i}`} className="flex items-center gap-2 px-6 text-sm font-mono">
